...
c) 字体：有明确要求的按要求执行，没有明确要求按文档模板执行，文档标题及页眉需匹配项目名称；`;
  ...
  if (selectedProposal) {
    ...
    if (workspaceMode) {
      ...
      // Helper to render section content with requirement hints, images, and body text
        ...
        return (
          <div className="space-y-1.5 mb-3">
            {lines.map((line, idx) => {
              ...
              // Check for image URLs in the line
                ...
                return (
                  <div key={idx}>
                    {isRequirement ? (
                      ...
                      <p className="text-sm text-foreground/80 leading-relaxed">{trimmed.replace(imageUrlPattern, "").trim()}</p>
                    )}
              ...
              return <p key={idx} className="text-sm text-foreground/80 leading-relaxed">{trimmed}</p>;
            })}
          </div>
        );
      };

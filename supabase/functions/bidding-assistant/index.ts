...
c) 字体：有明确要求的按要求执行，没有明确要求按文档模板执行，文档标题及页眉需匹配项目名称；`;
  ...
  // Build concise scoring text (avoid raw JSON dump)
  const scoringTextConcise = bid.scoring_table && Array.isArray(bid.scoring_table) && bid.scoring_table.length > 0
    ? bid.scoring_table.map((s: any, i: number) => `${i + 1}. ${s.item || s.name || s.category || "项目"}: ${s.score || s.weight || s.points || ""}分 ${s.description || s.criteria || ""}`).join("\n")
    : "（无评分标准数据）";

  // Limit docs to 20 most relevant, keep summary short
  const relevantDocs = (docs || []).slice(0, 20);
  const docsText = relevantDocs.map((d: any) => `- ${d.file_name} [${d.doc_category || ""}]`).join("\n") || "（无）";

  // Limit employees to 15
  const relevantEmployees = (employees || []).slice(0, 15);
  const employeesText = relevantEmployees.map((e: any) => `- ${e.name}: ${e.current_position || ""}, 证书: ${(e.certifications || []).join(",")}`).join("\n") || "（无）";
